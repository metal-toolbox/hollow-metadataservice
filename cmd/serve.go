...
func initDB() *sqlx.DB {
	...
	sqldb, err := otelsql.Open(dbDriverName, viper.GetString("db.uri"), otelsql.WithTracerProvider(tracerProvider))
	if err != nil {
	...
	db.SetMaxIdleConns(viper.GetInt("db.connections.max_idel"))
