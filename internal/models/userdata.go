...
var UserdatumWhere = struct {
	...
	ID:        whereHelperstring{field: "\"userdata\".\"id\""},
	Userdata:  whereHelpernull_Bytes{field: "\"userdata\".\"userdata\""},
	CreatedAt: whereHelpertime_Time{field: "\"userdata\".\"created_at\""},
	UpdatedAt: whereHelpertime_Time{field: "\"userdata\".\"updated_at\""},
}
...
var (
	userdatumAllColumns            = []string{"id", "userdata", "created_at", "updated_at"}
	userdatumColumnsWithoutDefault = []string{"id", "created_at", "updated_at"}
	userdatumColumnsWithDefault    = []string{"userdata"}
	userdatumPrimaryKeyColumns     = []string{"id"}
...
func (q userdatumQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Userdatum, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for userdata")
	}
...
func (q userdatumQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count userdata rows")
	}
...
func (q userdatumQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if userdata exists")
	}
...
func Userdata(mods ...qm.QueryMod) userdatumQuery {
	mods = append(mods, qm.From("\"userdata\""))
	q := NewQuery(mods...)
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"userdata\".*"})
	}
...
func FindUserdatum(ctx context.Context, exec boil.ContextExecutor, iD string, selectCols ...string) (*Userdatum, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"userdata\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from userdata")
	}
...
func (o *Userdatum) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no userdata provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"userdata\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"userdata\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into userdata")
	}
...
func (o *Userdatum) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update userdata, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"userdata\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update userdata row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for userdata")
	}
...
func (q userdatumQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for userdata")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for userdata")
	}
...
func (o UserdatumSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"userdata\" SET %s WHERE %s",
...
func (o *Userdatum) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"userdata\" WHERE \"id\"=$1"
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from userdata")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for userdata")
	}
...
func (q userdatumQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from userdata")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for userdata")
	}
...
func (o UserdatumSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"userdata\" WHERE " +
...
func (o *Userdatum) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindUserdatum(ctx, exec, o.ID)
	if err != nil {
		return err
	}
...
func (o *UserdatumSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"userdata\".* FROM \"userdata\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, userdatumPrimaryKeyColumns, len(*o))
...
func UserdatumExists(ctx context.Context, exec boil.ContextExecutor, iD string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"userdata\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if userdata exists")
	}
...
func (o *Userdatum) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no userdata provided for upsert")
	}
	if !boil.TimestampsAreSkipped(ctx) {
		currTime := time.Now().In(boil.GetLocation())
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert userdata, could not build update column list")
		}
		...
		cache.query = buildUpsertQueryCockroachDB(dialect, "\"userdata\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert userdata")
	}
